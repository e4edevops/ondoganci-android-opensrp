...
public class HIA2IntentService extends IntentService {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        dailyTalliesRepository = OndoganciApplication.getInstance().dailyTalliesRepository();
        monthlyTalliesRepository = OndoganciApplication.getInstance().monthlyTalliesRepository();
        hia2ReportRepository = OndoganciApplication.getInstance().hia2ReportRepository();
        hia2Service = new HIA2Service();

        vaccineRepository = OndoganciApplication.getInstance().vaccineRepository();
    ...
    private void generateDailyHia2Indicators() {
        try {
            ...
            String lastProcessedDate = OndoganciApplication.getInstance().context().allSharedPreferences().getPreference(HIA2Service.HIA2_LAST_PROCESSED_DATE);
            ...
            for (Map<String, String> dates : reportDates) {
                ...
                OndoganciApplication.getInstance().context().allSharedPreferences().savePreference(HIA2Service.HIA2_LAST_PROCESSED_DATE, updatedAt);
            }
        } catch (Exception e) {
            Log.e(TAG, e.getMessage(), e);
        }
    ...
    private Long countDaysAfterDueDate(Vaccine vaccine) {
        ...
        CommonRepository commonRepository = OndoganciApplication.getInstance().context().commonrepository(org.smartregister.child.util.Utils.metadata().childRegister.tableName);
