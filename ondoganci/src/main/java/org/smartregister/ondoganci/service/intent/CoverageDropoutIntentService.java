...
import org.smartregister.ondoganci.domain.CohortPatient;
import org.smartregister.ondoganci.domain.Cumulative;
import org.smartregister.ondoganci.domain.CumulativeIndicator;
import org.smartregister.ondoganci.domain.CumulativePatient;
...
public class CoverageDropoutIntentService extends IntentService {
    ...
    private static final String BIRTH_REGISTRATION_QUERY = "SELECT " +
            ...
            " FROM " + Utils.metadata().childRegister.tableName +
            " WHERE " + DOD_FILTER;
    ...
    private static final String VACCINE_QUERY = "SELECT " +
            ...
            " INNER JOIN " + Utils.metadata().childRegister.tableName + " c  " +
    ...
    private void generateBirthRegistrationIndicators() {
        try {
            EventClientRepository eventClientRepository = OndoganciApplication.getInstance().eventClientRepository();
            ...
            SQLiteDatabase db = OndoganciApplication.getInstance().getRepository().getWritableDatabase();
            String lastProcessedDate = OndoganciApplication.getInstance().context().allSharedPreferences().getPreference(COVERAGE_DROPOUT_BIRTH_REGISTRATION_LAST_PROCESSED_DATE);
            ...
            for (Map<String, String> result : results) {
                ...
                OndoganciApplication.getInstance().context().allSharedPreferences().savePreference(COVERAGE_DROPOUT_BIRTH_REGISTRATION_LAST_PROCESSED_DATE, updatedAt);
            }
    ...
    private void generateVaccineIndicators() {
        try {
            ...
            SQLiteDatabase db = OndoganciApplication.getInstance().getRepository().getWritableDatabase();
            String lastProcessedDate = OndoganciApplication.getInstance().context().allSharedPreferences().getPreference(COVERAGE_DROPOUT_VACCINATION_LAST_PROCESSED_DATE);
            ...
            for (Map<String, String> result : results) {
                try {
                    ...
                    OndoganciApplication.getInstance().context().allSharedPreferences().savePreference(COVERAGE_DROPOUT_VACCINATION_LAST_PROCESSED_DATE, updatedAt);
                } catch (Exception e) {
                    Log.e(TAG, e.getMessage(), e);
                }
            }
    ...
    private static void updateCumulativeRegistrations(String baseEntityId, Date vaccineDate) {
        ...
        CumulativePatientRepository cumulativePatientRepository = OndoganciApplication.getInstance().cumulativePatientRepository();
