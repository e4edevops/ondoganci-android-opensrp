...
public class PullUniqueIdsIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        try {
            ...
            if (uniqueIdRepo.countUnUsedIds() == 0) { // first time pull no ids at all
                numberToGenerate = AppConstants.OPENMRS_UNIQUE_ID_INITIAL_BATCH_SIZE;
            } else if (uniqueIdRepo.countUnUsedIds() <= 250) { //maintain a minimum of 250 else skip this pull
                numberToGenerate = AppConstants.OPENMRS_UNIQUE_ID_BATCH_SIZE;
            } else {
                return;
            }
            JSONObject ids = fetchOpenMRSIds(AppConstants.OPENMRS_UNIQUE_ID_SOURCE, numberToGenerate);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        uniqueIdRepo = OndoganciApplication.getInstance().uniqueIdRepository();
