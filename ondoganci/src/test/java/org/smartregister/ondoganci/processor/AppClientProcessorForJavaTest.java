...
@PrepareForTest({OndoganciApplication.class, Utils.class, VaccineSchedule.class, ServiceSchedule.class, ChildDbUtils.class, ChildLibrary.class})
public class AppClientProcessorForJavaTest {
    ...
    public void processHeightWithValidEventClientAndTableShouldReturnTrue() throws Exception {
        ...
        PowerMockito.when(ondoganciApplication.heightRepository()).thenReturn(heightRepository);
    ...
    public void processServiceWithNameNotHavingITNShouldReturnTrue() throws Exception {
        PowerMockito.mockStatic(OndoganciApplication.class);
        PowerMockito.when(OndoganciApplication.getInstance()).thenReturn(ondoganciApplication);
        PowerMockito.when(ondoganciApplication.recurringServiceTypeRepository()).thenReturn(recurringServiceTypeRepository);
        PowerMockito.when(ondoganciApplication.recurringServiceRecordRepository()).thenReturn(recurringServiceRecordRepository);
    ...
    public void processBCScarEventWithValidEventClientShouldPassCorrectArgsToDetailsRepo() throws Exception {
        PowerMockito.mockStatic(OndoganciApplication.class);
        PowerMockito.when(OndoganciApplication.getInstance()).thenReturn(ondoganciApplication);
        Mockito.when(ondoganciApplication.context()).thenReturn(openSrpContext);
    ...
    public void processWeightWithValidEventClientAndTableShouldReturnTrue() throws Exception {
        PowerMockito.mockStatic(OndoganciApplication.class);
        PowerMockito.when(OndoganciApplication.getInstance()).thenReturn(ondoganciApplication);
        PowerMockito.when(ondoganciApplication.weightRepository()).thenReturn(weightRepository);
        Mockito.when(processorForJava.processCaseModel(ArgumentMatchers.any(EventClient.class), ArgumentMatchers.any(Table.class))).thenReturn(contentValues);
        Mockito.when(contentValues.size()).thenReturn(7);
